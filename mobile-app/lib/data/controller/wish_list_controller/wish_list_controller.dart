...
class WishListController extends GetxController {
  ...
  void removeFromWishlist(int index) async {
    ...
    ResponseModel model = await repo.removeFromWishlist(
        int.tryParse(wishlist[index].itemId ?? '0') ?? 0, int.tryParse(wishlist[index].episodeId ?? '0') ?? 0);
    ...
    if (model.statusCode == 200) {
      ...
      if (responseModel.status == 'success') {
        ...
        CustomSnackbar.showCustomSnackbar(
            errorList: [], msg: [responseModel.message?.success ?? 'Successfully removed'], isError: false);
        removeLoading = false;
        update();
      } else {
        CustomSnackbar.showCustomSnackbar(
            errorList: [responseModel.message?.error ?? 'Something went wrong'], msg: [], isError: true);
        removeLoading = false;
        update();
      }
    } else {
