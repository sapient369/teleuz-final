    ...
    if (responseModel.statusCode == 200) {
      ...
      if (model.message?.success != null && model.message!.success!.isNotEmpty) {
        CustomSnackbar.showCustomSnackbar(
            errorList: [], msg: model.message?.success ?? [MyStrings.passwordChanged], isError: false);
        return true;
      } else {
        CustomSnackbar.showCustomSnackbar(
            errorList: model.message?.error ?? [MyStrings.failedToChangedPassword], msg: [], isError: true);
        return false;
      }
    } else {
      return false;
