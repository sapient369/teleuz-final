...
class UserController extends Controller {
    ...
    public function watchTelevision($id = 0) {
        $tv = LiveTelevision::with('categories')->active()->where('id', $id)->first();
        if (!$tv) {
            return response()->json([
                'remark'  => 'not_found',
                'status'  => 'error',
        ...
        $paidCategories = $tv->categories->where('price', '>', 0)->pluck('id');
        ...
        $notify[]  = $tv->title;
        $relatedTv = LiveTelevision::whereHas('categories', function ($query) {
            $query->active();
