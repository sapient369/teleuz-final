...
class SiteController extends Controller {
    ...
    public function liveTelevision($id = 0) {
        ...
        $channelCategories = ChannelCategory::active()
            ...
            ->withWhereHas('channels', function ($query) {
                $query->active();
            })->get();
        return view('Template::live_tvs', compact('pageTitle', 'channelCategories'));
    }
    ...
    public function watchTelevision($id) {
        ...
        $tv   = LiveTelevision::with('categories')->active()->findOrFail($id);
        $user = auth()->user();

        $adultCategory = $tv->categories->firstWhere('is_adult', 1);
        if ($adultCategory && !session('adult_verified')) {
            session()->put('url.intended', url()->current());
            session()->put('adult.category_id', $adultCategory->id);            
            return redirect()->route('user.adult.password');
        }

        $needSubscriptionIds = $tv->categories
            ->filter(function ($cat) {
                return $cat->is_adult || $cat->price > 0;
            })
            ->pluck('id');

        if ($needSubscriptionIds->count() > 0) {
            $hasSubscribed = Subscription::where('user_id', $user->id)
                ->whereIn('channel_category_id', $needSubscriptionIds)
        ...
        $pageTitle = $tv->title;
        $otherTvs  = LiveTelevision::whereHas('categories', function ($query) {
            $query->active()->where('is_adult', 0);
        })
