            ...
            @foreach ($channelCategories as $category)
                ...
                <div class="tv-live py-4">
                    ...
                    <div class="tv-card-wrapper">
                        @if ($category->is_adult && !session('adult_verified'))
                            <a href="{{ route('user.adult.password', ['category' => $category->id]) }}" class="btn btn--base">@lang('Unlock with PIN')</a>
                        @elseif($category->is_adult && !$eligable)
                            <p class="text-muted">@lang('Subscribe to access this category')</p>
                        @else
